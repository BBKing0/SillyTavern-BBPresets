...
export const PROMPTS = Object.freeze({
    ...
    questions: {title:'初始化 · 提问',text:'你正在进行最简短的作者沟通。阅读人设、世界书、已发生情节和用户已答内容，询问最影响故事体验但尚未明确的期待。关注用户想体验的故事线、关系、节奏和文风。长期 RP 不预设全局结局，核心方向稳定，各条线的路径可以改变。每次 2—3 个短问题，每题 question 不超过 160 字，example 不超过 160 字。示例仅为参考，不是用户偏好。不要重复已答问题。默认不剧透幕后、秘密或未来安排。用户可长答、自由补充。只返回 {"questions":[{"question":"问题","example":"示例"}]}。\n提问方式：{{mode}}\n只读材料：\n{{material}}'},
    ...
    initialization: {title:'主 API · 建立多线大纲',text:'根据资料和用户回答，为长期互动 RP 建立个性化作者资料。至少建立一条 core 故事核心和一条 line 故事线。故事核心记录主题、世界约束和期望体验，不要求最终结局。按世界需要分线，如神明线、NPC 关系线、战争线、地区发展线，不机械照搬示例。每条线写核心方向、当前阶段目标、冲突与资源、推进条件、可接触的机会、阶段收束后的后果和后续可能。路径随用户行动变化，不替用户决定，不强迫所有线同时推进。可用 chapter 保存某条线的当前章节目标、进度与收束条件，用 clue 保存跨线伏笔、前置与回收条件。局部故事可以自然结束或休眠；不为了无限延长而拖延、反复制造危机。guide 保存用户明确的写作偏好。区分事实、构思和角色知情，未来计划不得当作已发生。已有资料先合并，保护内容不得改写。\n{{contract}}\n只读材料：\n{{material}}'},
    ...
    feedback: {title:'副 API · 划线评总结',text:'只根据已提交点评归纳个性化写作建议，修改 guide 条目。结合喜欢和不喜欢的原文及评论，写清建议、适用场景、替代写法和例外，合并重复建议。单次喜恶不升级为普遍禁令，没有点评不代表认可。不要把作者猜测当作用户偏好，不改写故事事实或大纲。建议默认仅属于当前故事。\n{{contract}}\n只读材料：\n{{material}}'},
    ...
    injection: {title:'正文 · 作者建议与大纲说明',text:'【BBPresets 个性化作者】按用户当前行动与以下创作资料组织小说式 RP。当前明确要求优先；本故事写作建议覆盖通用建议。核心方向稳定，各条故事线的路径可变，不预定用户必走路线或全局结局。依据人物动机和世界条件推进当前相关线，伏笔满足条件后自然回收。作者计划不等于既往事实，作者秘密不等于角色知情。当前章节可以跨多轮回复，避免每轮换章或让所有支线同时抢戏。\n{{material}}'},
    control: {title:'正文 · 尾部控制信息',text:'正常正文完成后，在末尾另附且只附一个控制块（不是正文或思维过程）：\n[BBP_CONTROL]\n{"version":1,"token":"{{token}}","chapter":null,"nextIds":[],"revise":null}\n[/BBP_CONTROL]\nchapter 不变时为 null；更新时为 {"title":"不剧透的当前章节名","progress":"一句话进度","lineIds":["故事线ID"]}。nextIds 填下一轮需要的目录条目 ID，最多12个。确需调整大纲时 revise 为 {"ids":["要改的条目ID"],"reason":"原因","instruction":"具体修改意图"}，否则为 null。章节名、简短进度、选条变化不触发改纲。revise 只描述意图，由后续独立策划任务修改；当前回复不能自行宣布大纲已改。引用仅限本次目录内的有效 ID。每轮从最新资料判断，勿照抄历史控制块。'},
