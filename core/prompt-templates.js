...
export const PROMPTS = Object.freeze({
    ...
    outline: {title:'主 API · 按需修订故事线',text:'依据用户要求或当前回复的改纲信号，修订相关故事线、章节与伏笔。核心方向稳定，具体路径可随互动变化；长期 RP 无需总终局，各线可推进、交汇、收束、休眠或自然产生后续。仅修改必要条目，核对关联线的因果关系，不把计划写成事实，不代替用户行动。不必为剧情目标的延续或下轮选条重写整份大纲。只修改 core/line/chapter/clue，不改写读者偏好。\n{{contract}}\n只读材料：\n{{material}}'},
    ...
    injection: {title:'正文 · 作者建议与大纲说明',text:'【BBPresets 个性化作者】按用户当前行动与以下创作资料组织小说式 RP。写作建议随作者沿用。chapter.progress 是尚在推进的剧情目标与下一步方向，不是上一段总结；围绕同一目标自然发展，可跨多轮揭露、追问与回应，不为每次回复换目标。故事核心方向稳定、路径可变，不预定用户必走路线或全局结局。core/outlines 仅在需要查阅时提供，空数组是正常状态；沿用当前目标、对话和人物动机即可，不因缺少全文而停顿、补齐所有线或制造转折。directory 只是部分可用条目的简短索引，不是本轮待办清单。大纲主要在转折、目标收束、新线索或方向不明时帮助选择后续发展。作者计划不等于事实，作者秘密不等于角色知情。\n{{material}}'},
    control: {title:'正文 · 尾部控制信息',text:'正常正文完成后，在末尾另附且只附一个控制块：\n[BBP_CONTROL]\n{"version":2,"token":"{{token}}","chapter":null,"nextIds":[],"revise":null}\n[/BBP_CONTROL]\nchapter 更新格式为 {"progress":"当前剧情目标与接下来要展开的方向","lineIds":["关联故事线ID"]}，不输出 title。首次目标为空时从当前场景建立目标；目标未完成时 chapter:null 表示继续沿用，也可原样返回同一目标。仅在目标完成、剧情转折或用户改变方向时调整，不逐轮总结刚发生的事，不为了换目标强行加速。例：progress 为“user和char对峙，char揭露自己真实目的”；第一轮 char 揭露目的，第二轮 user 追问、char 回答，两轮沿用同一 progress。目标允许起伏、铺垫和自然收束，不替 user 决定行动。lineIds 仅表示目标归属，不会自动调用全文。\nnextIds 默认 []；日常对话、追问、情绪反应和同一目标内的延续通常不需大纲全文。仅预见下一轮存在转折、目标收束、关键伏笔或确实需要确认方向时，按必要程度选择最少的有效条目，通常1—2条；程序按设置限制调用数量。一次选择仅供下一轮，读过后继续推进通常应回到 []。勿为保险把核心、关联线和所有目录一起选入。\n确需实质修订大纲时 revise 为 {"ids":["要改的条目ID"],"reason":"原因","instruction":"具体修改意图"}，否则 null。目标延续或变化、选条不触发改纲；revise 由后续独立任务执行，本轮不能宣布已改。所有引用限本次提供的目录、正文条目或目标 lineIds 中的有效 ID。'},
